...
class Scraper:
    ...
    def scrape_discount(self, discount_id, category_id, session):
        page = self.requests_session.get(Scraper.DISCOUNT_URL, params={"id": discount_id})
        ...
        if len(soup.find_all("finish")) > 0:
            ...
            raise RuntimeError
        discount = Discount(discount_id=discount_id, category_id=category_id)
        ...
        try:
            ...
            discount.end_date = discount.start_date.replace()
        discount.image_url = soup.find("img", class_=["img_big", "img_big2"]).get("src")
        return discount

    def scrape_category(self, category_id, session):
        cat_type = int(str(category_id)[0])
        cat_num = int(str(category_id)[1:])
        if cat_type == 1:
        ...
        found_discounts = []
        while 1:
            page = self.requests_session.get(Scraper.CATEGORY_URL, params=payload)
            soup = BeautifulSoup(page.content, "html.parser")
            for sale_table_item in soup.find_all("table", class_="sale_table"):
                discount_id = int(sale_table_item.find("a").get("href").split("=")[-1])
                discount_exists = session.query(exists().where(Discount.discount_id == discount_id)).scalar()
                if not discount_exists:
                    found_discounts.append(self.scrape_discount(discount_id, category_id, session))
    ...
    def scrape_all_categories(self, session):
    ...
    def scrape_discounts(self):
        ...
        new_discounts = self.scrape_all_categories(session)
    ...
    def scrape_categories():
        page = requests.get(Scraper.ALL_CATEGORIES_URL)
        soup = BeautifulSoup(page.content, "html.parser")
        ...
        for sfmenu in groceries_sfmenu, goods_sfmenu:
            ...
            for li in sfmenu.find_all("li"):
                ...
                if category_id not in categories:
                    categories[category_id] = Category(category_id=category_id, name=name)
                ...
                if parent_li.name == "li":  # Category has a parent category
                    ...
                    categories[category_id].parents.append(parent_category)
        return categories.values()

    @staticmethod
    def scrape_shops():
        shops = []
        page = requests.get(Scraper.ALL_SHOPS_URL)
        soup = BeautifulSoup(page.content, "html.parser")
        p_list = soup.find_all("p", class_=["shop", "shop_mx"])
        for p in p_list:
            name = p.text
            a = p.parent.find("a")
            image_url = Scraper.BASE_URL + "/" + a.find("img").get("src")
            # This is necessary to get the shop_id because it's not in the shop list HTML
            shop_url = Scraper.BASE_URL + "/" + a.get("href")
